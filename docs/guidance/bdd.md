...
> themselves.** If BA/PO merely *read* scenarios for sign-off (the common case), prefer
...
making them readable by non-technical stakeholders. This framework uses [`playwright-bdd`](https://vitalets.github.io/playwright-bdd/)
because it compiles `.feature` files into Playwright test specs — so BDD runs on the **same** Playwright runner and
reuses the **same** custom fixtures, page objects, and services already in this repo.
...
- **One step, one intent.** `When the user signs in as {string}` is reusable; `When the user types email and clicks the
  blue button` is not.
